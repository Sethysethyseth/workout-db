...
- DONE  dead-zone fix: widened each palette's Home ambient glow reach
        ...
        quick-picks section (max-data anti-goal). Needs smoke-test confirm.

## Next up (the active task)

Smoke-test the dead-zone fix (5 palettes x dark x Home) - confirm the glow
now bleeds toward the middle and doesn't look overdone. If it holds up,
ui-palettes-v2 -> main is a manual merge (ask first).
