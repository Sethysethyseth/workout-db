/**
 * Optional non-negative decimal with a maximum number of fractional digits.
 * Does not round: values with more than maxDecimals fractional digits are rejected.
 *
 * @param {unknown} value
 * @param {{ maxDecimals: number, fieldName: string, allowZero?: boolean }} opts
 * @returns {{ ok: true, value: number | null } | { ok: false, error: string }}
 */
function validateOptionalNonNegDecimal(value, opts) {
